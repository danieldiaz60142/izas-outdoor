  ...
  class CartItems extends HTMLElement {
    ...
    async updateQuantity(line, quantity, name) {
      ...
      try {
        ...
        if (this.cartDrawer && newTotalQuantity === 0) {
          this.cartDrawer
            .querySelector('.cart-drawer__content')
            .classList.add('grow', 'flex', 'items-center');
    ...
    async refreshCart() {
      ...
      try {
        ...
        const unique_sections = Array.from(new Set(sections));
        const response = await fetch(`?sections=${unique_sections}`);
