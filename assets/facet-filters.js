...
if (!customElements.get('facet-filters')) {
  class FacetFilters extends SideDrawer {
    ...
    handleFilterChange(evt) {
      ...
      // Ensure the availability filter is always included
    ...
    handleFiltersClick(evt) {
      ...
      if (target.matches('.js-clear-filter')) {
        ...
        searchParams.set('filter.v.availability', '1'); // Ensure the availability filter is included
        this.applyFilters(searchParams.toString(), evt);
      }
    ...
    handleActiveFiltersClick(evt) {
      ...
      evt.preventDefault();
      const searchParams = new URL(evt.target.href).searchParams;
      searchParams.set('filter.v.availability', '1'); // Ensure the availability filter is included
      this.applyFilters(searchParams.toString(), evt);
    }
    ...
    async applyFilters(searchParams, evt, updateUrl = true) {
      ...
      // Ensure the availability filter is always included in the URL
    ...
    static updateFilter(filter, fetchedFilter, updateAll) {
      if (fetchedFilter) {
        ...
        if (updateAll) {
          ...
          if(fetchedFilter.querySelector('.filter__toggle') && filter.querySelector('.filter__toggle'))  {
            filter.replaceChild(
                fetchedFilter.querySelector('.filter__toggle'),
                filter.querySelector('.filter__toggle')
            );
          }
          if(fetchedFilter.querySelector('.filter__header')) {
