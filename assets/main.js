...
class ProductCard extends HTMLElement {
  ...
  triggerSwatch(evt) {
    let cardSwatches = evt.querySelector('.card__swatches')
    if(cardSwatches.querySelector('input[data-checked="true"]')) {
      cardSwatches.querySelector('input[data-checked="true"]').click();
      if(window.theme.settings.showPdPBottom && window.theme.settings.paginationInfinite) {
        let statusAvailable =  cardSwatches.querySelector('input[data-checked="true"]').getAttribute('data-available')
        if (statusAvailable == 'false') {
          if(!cardSwatches.closest('product-card').closest('li').classList.contains('product-out-stock')) {
            cardSwatches.closest('product-card').closest('li').classList.add('product-out-stock')
          }
          if(cardSwatches.closest('product-card')) {
            cardSwatches.closest('product-card').classList.add('variant-out-stock');
          }
          if(document.querySelector('.js-pagination')) {
            if(!cardSwatches.closest('product-card').closest('li').classList.contains('hidden')) {
              cardSwatches.closest('product-card').closest('li').classList.add('hidden')
            }
          } else {
            cardSwatches.closest('product-card').closest('li').classList.remove('hidden')
          }

          if(cardSwatches.closest('product-card').closest('li').classList.contains('product-in-stock')) {
            cardSwatches.closest('product-card').closest('li').classList.remove('product-in-stock')
          }
        }
      }
    }
...
class StoreHeader extends HTMLElement {
  ...
  bindEvents() {
    ...
    if (this.dataset.isSearchMinimised) {
      if (this.searchToggle) {
        ...
        const annountHeightcss = document.querySelector('.cc-announcement announcement-bar'); 
        if (annountHeightcss){
          document.documentElement.style.setProperty('--header-annount-height', `${annountHeightcss.offsetHeight}px`);
        } else{
          document.documentElement.style.setProperty('--header-annount-height', `0px`);
        }
        const headerHeightcss = document.querySelector('.shopify-section-group-header-group.cc-header'); 
        if (headerHeightcss){
          document.documentElement.style.setProperty('--header-cs-height', `${headerHeightcss.offsetHeight}px`);
        } else{
          document.documentElement.style.setProperty('--header-cs-height', `0px`);
        }
      }
  ...
  handleSearchToggleClick(evt) {
    ...
    const dataAlign =  document.querySelector('[data-header-align]'); 
    const overflowHidden =  document.querySelector('body.cs-overflow-hidden'); 

    if(!overflowHidden){
      if (dataAlign){
        document.body.classList.add('cs-overflow-hidden');
      }
    } else {
...
handleScroll() {
  ...
  if (!document.body.classList.contains('fixed') && !this.stickyTransitioning) {
      ...
      this.hide();
      this.addScrollClass(); // Add the class when scrolling down
    } else if (this.lastScrollPos - 5 > scrollTop) {
      this.show();
    }
  ...
  // Add the following condition to check if the user has scrolled to the top
  if (scrollTop === 0) {
    this.scrollReachedTop();
  }
}
...
class renderPriceSale extends HTMLElement {
  ...
  updatePrices() {
    let priceRange = this.querySelector('.price-sale-tag');
    let parentElement = priceRange.parentElement.parentElement.parentElement;
    if (parentElement && parentElement.classList.contains('card__info-inner')) {
      let allColorOptions = parentElement.querySelectorAll('.js-option');
      if (allColorOptions && allColorOptions.length > 0) {
        allColorOptions.forEach((evl) => {
          if (evl.checked) {
            ...
            if (comparePrice && comparePrice != null && comparePrice != undefined) {
              if (parseFloat(price) < parseFloat(comparePrice) && evl.dataset.compareformat != null && evl.dataset.compareformat != 'undefined') {
                ...
                if (divPriceDiscount) {
                  ...
                  divPriceDiscount.innerHTML = `Ahorra ${discount}%`;
                }
              } else {
                if (divPriceWas) {
      ...
      if (priceRange && priceRange != null) {
        let comparePrice = priceRange.dataset.compare;
        let price = priceRange.dataset.price;
        if (comparePrice && comparePrice != null && comparePrice != undefined) {
